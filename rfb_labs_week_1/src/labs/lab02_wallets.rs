...
pub fn list_wallets<C: RpcClient>(client: &C) -> LabResult<Vec<String>> {
    ...
    let array = value
        .as_array()
        .ok_or_else(|| LabError::Parse("expected array from listwallets".to_owned()))?;
    array
        .iter()
        .map(|v| {
            v.as_str()
                .map(ToOwned::to_owned)
                .ok_or_else(|| LabError::Parse("wallet name is not a string".to_owned()))
        })
        .collect()
}
...
pub fn get_new_address<C: RpcClient>(
    ...
    value
        .as_str()
        .map(ToOwned::to_owned)
        .ok_or_else(|| LabError::Parse(format!("expected address string, got: {value}")))
}
